...
| `no available target` | 路由无可用目标 | 检查 Token 是否同步、候选目标是否处于冷却或不可用状态；可在路由页查看候选状态 |
| `target cooling` | 目标进入冷却期 | 目标在请求失败后自动冷却，期间不会被路由选中；无需干预，会自动恢复 |
| `upstream 429` | 上游限流 | 该上游站点触发了速率限制；路由引擎会自动切换其他候选目标，冷却期后重试 |
| `upstream 5xx` | 上游服务器错误 | 上游站点临时不可用，路由引擎会自动故障转移到其他候选目标 |
...
| `proxy timeout` | 代理请求超时 | 上游响应过慢；检查网络延迟或考虑切换其他候选目标 |
...
| 操作 | 位置 | 适用场景 |
|------|------|----------|
| **清除缓存并重建路由** | 设置 → 清除缓存并重建路由 | 全局刷新：清空模型发现缓存和自动路由，后台触发模型刷新与路由重建 |
