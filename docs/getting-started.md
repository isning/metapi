...
- 如果这个站点的控制台 URL 和真实 API 请求地址不同，不要直接把主站点 URL 改掉，而是在 endpoint profile 中配置实际请求地址。

> [!IMPORTANT]
> 通用平台常见写法是先填控制台地址或 provider URL；如果上游的真实请求路径不同，应在 endpoint profile 中保存完整请求地址。官方预设带出的 URL 不要手动增删 `/v1`、`/anthropic` 或 `/api/coding/...`。
