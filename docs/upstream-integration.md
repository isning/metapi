...
| 只有请求 URL + API Key 的兼容接口 | 站点管理 | 先加站点，再加 API Key，并在 endpoint profile 中确认实际请求地址 |
...
| DeepSeek / OpenAI | `openai` | `https://api.deepseek.com` | 适合直接接 DeepSeek 官方 OpenAI 兼容入口 |
| DeepSeek / Claude | `claude` | `https://api.deepseek.com/anthropic` | 适合 Claude / Anthropic Messages 兼容用法 |
...
3. 对官方预设来说，**不要机械地增删 `/v1`、`/anthropic` 或 `/api/coding/...`**，以预设默认值为准
4. 对支持 endpoint profile 的站点，实际代理调用以 endpoint profile 中保存的完整请求地址为准。站点 URL 主要用于站点管理、默认配置和目录发现。

DeepSeek 这类同时支持 OpenAI 和 Anthropic 兼容入口的站点，推荐建模为一个站点、一个共享模型目录和多个 endpoint profile。详细说明见 [上游 Endpoint、模型目录与兼容性](./upstream-endpoint-compatibility.md)。
...
- **Endpoint Profile 请求地址**：用于真正转发 `/v1/*`、`/chat/completions`、`/responses`、`/messages` 等模型请求
- **Model Catalog Source**：用于刷新模型列表，例如 `GET /models`

适合配置 endpoint profile 的场景：

1. 控制台地址和 API 网关地址不同
2. 同一个站点同时支持 OpenAI Chat、Responses 或 Anthropic Messages
3. 模型目录地址和真实请求地址不同
4. 你想保留面板地址不变，只把模型流量导向专用网关

简单理解：

```text
主站点 URL = 控制面
Endpoint Profile = 数据面请求入口
Model Catalog Source = 模型目录入口
```

完整建模见 [上游 Endpoint、模型目录与兼容性](./upstream-endpoint-compatibility.md)。
...
4. 主站点 URL 和真实 API 请求地址不同，但还没配置 endpoint profile
...
- 不要在账号管理里反复切换 Cookie / Session / API Key 认证方式
...
- **官方预设站点：** 新增模型时同步检查推荐模型、endpoint profile 和模型目录来源
