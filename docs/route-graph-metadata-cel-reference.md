...
| `selectExpression` | `direct` | 返回 option index 或含 `idx` 的对象 |
...
Direct expression 返回无效 index 时，selector 会选择当前 eligible 列表中的第一个 option。因此 direct policy 应配合 eligibility expression，且不要用它模拟 fallback stage。
