...
describe('harness workflows', () => {
  ...
  it('keeps the comprehensive test matrix issue slices publishable', () => {
    const issueSet = readFileSync(resolve(process.cwd(), 'docs/plans/2026-06-18-comprehensive-test-matrix-issues.md'), 'utf8');
    const matrix = readFileSync(resolve(process.cwd(), 'docs/plans/2026-06-18-comprehensive-test-matrix.md'), 'utf8');
    const localIssueFiles = [
      '.scratch/comprehensive-test-matrix/01-test-harness-adoption-and-fixture-governance.md',
      '.scratch/comprehensive-test-matrix/02-route-graph-compiler-and-runtime-coverage.md',
      '.scratch/comprehensive-test-matrix/03-macro-and-automatic-route-construction-coverage.md',
      '.scratch/comprehensive-test-matrix/04-dispatcher-token-router-and-downstream-key-routing-coverage.md',
      '.scratch/comprehensive-test-matrix/05-proxy-relay-protocol-coverage.md',
      '.scratch/comprehensive-test-matrix/06-admin-api-database-and-migration-coverage.md',
      '.scratch/comprehensive-test-matrix/07-frontend-route-graph-editor-and-route-wizard-coverage.md',
      '.scratch/comprehensive-test-matrix/08-admin-ui-pages-and-model-tester-coverage.md',
      '.scratch/comprehensive-test-matrix/09-critical-e2e-workflows-and-ci-reporting.md',
    ];

    expect(issueSet.match(/^## Issue \d+:/gm) || []).toHaveLength(9);
    expect(matrix).toContain('## Final Gate');

    for (const path of localIssueFiles) {
      const issue = readFileSync(resolve(process.cwd(), path), 'utf8');
      expect(issue).toContain('## What to build');
      expect(issue).toContain('## Acceptance criteria');
      expect(issue).toContain('## Blocked by');
    }
  });
});
