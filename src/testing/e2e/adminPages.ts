...
export async function expectRouteEditorModes(page: Page): Promise<void> {
  ...
  await expect(page.getByRole('tab', { name: /^Route Graph$|^路由图$/i })).toBeVisible();
  await expect(page.getByRole('tab', { name: /^Export JSON$|^导出 JSON$/i })).toBeVisible();
  await expect(page.getByText(/Route groups|路由组/i).first()).toBeVisible();

  await page.getByRole('tab', { name: /^Route Graph$|^路由图$/i }).click();
