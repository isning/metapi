...
test("operates native Route Groups through the browser and persists each mutation", async ({
  ...
  const alphaSourceModel = alphaRow.getByLabel(/Upstream model|上游模型/i);
  await expect(alphaSourceModel).toBeVisible();
  await alphaSourceModel.fill(`${seeded.modelName}-alpha`);
  await alphaSourceModel.blur();
  await expect
    .poll(async () => {
      const current = await adminApi.getJson<{
        stages: Array<
          FallbackStage & {
            candidates: Array<
              FallbackStage["candidates"][number] & {
