...
const zhToEn: Record<string, string> = {
  ...
  '丢弃推理历史': 'Drop reasoning history',
  '最大推理字节数': 'Max reasoning bytes',
  '溢出处理': 'Overflow',
  '截断': 'Truncate',
  '丢弃': 'Drop',
  '工具调用消息': 'Tool-call messages',
  '跟随 assistant': 'Same as assistant',
  '强制原生字段': 'Force native',
  '丢弃推理': 'Drop reasoning',
  'Assistant 历史': 'Assistant history',
  'Assistant 工具调用': 'Assistant tool calls',
  '响应续写': 'Response continuation',
  ...
  '请先点击账号操作栏中的「刷新」或「模型」按钮获取模型。': 'Use Refresh or Model in the account action bar to fetch models first.',
  ...
  '全部启用': 'Enable All',
  '手动': 'Manual',
  '未配置成本': 'No cost pricing',
  '成本': 'Cost',
  '禁用的模型将对整个站点生效，该站点下所有连接都不会使用这些模型进行代理。': 'Disabled models apply to the whole site; no connection under this site will proxy those models.',
  '手动添加可用模型': 'Add Available Models Manually',
  '如果您的账号支持某些未在上方列表中显示的模型，可以在此手动添加（多个以英文逗号分隔）。': 'If this account supports models missing from the list, add them here separated by commas.',
  '例如: gpt-4-custom, claude-3-5-sonnet-20241022': 'Example: gpt-4-custom, claude-3-5-sonnet-20241022',
  '上游成本配置': 'Upstream Cost Pricing',
  '为当前账号模型配置 Metapi 调用上游供应的成本。': 'Configure the cost Metapi pays when this account model calls upstream supply.',
  'Token 组': 'Token Group',
  '未配置': 'Not configured',
  '上游模型成本': 'Upstream Model Cost',
  '加载成本配置失败': 'Failed to load cost pricing',
  '预览失败': 'Preview failed',
  '至少填写一个计价项': 'Enter at least one pricing item',
  '上游成本配置已保存': 'Upstream cost pricing saved',
  '保存成本配置失败': 'Failed to save cost pricing',
  '上游成本配置已删除': 'Upstream cost pricing deleted',
  '删除成本配置失败': 'Failed to delete cost pricing',
  '新建': 'New',
  '已配置': 'Configured',
  '按上游供给维度匹配': 'Matched by upstream supply scope',
  '加载中': 'Loading',
  '暂无成本配置': 'No cost pricing',
  '保存后会按作用域匹配上游成本。': 'After saving, upstream cost is matched by scope.',
  '计价表': 'Rate Card',
  '价格单位为 USD；token 项按每 1M tokens 计价': 'Prices are USD; token items are priced per 1M tokens.',
  '启用此配置': 'Enable this configuration',
  '关闭后保留配置，但运行时成本计算不会匹配它。': 'When disabled, the configuration is kept but runtime cost calculation will not match it.',
  '启用成本配置': 'Enable cost pricing',
  '作用域': 'Scope',
  '站点模型': 'Site Model',
  '账号模型': 'Account Model',
  'Token 模型': 'Token Model',
  'Token 分组模型': 'Token Group Model',
  '同一站点下同名模型共用，优先级最低。': 'Shared by the same model name under this site; lowest priority.',
  '仅当前账号下同名模型使用。': 'Only used by this model name under the current account.',
  '仅当前账号的指定 Token 使用。': 'Only used by the selected token under the current account.',
  '仅当前账号的指定 Token 分组使用，优先级最高。': 'Only used by the selected token group under the current account; highest priority.',
  '选择 token': 'Select token',
  '备注': 'Notes',
  '例如：供应商合同价、手动录入日期': 'Example: supplier contract price, manual entry date',
  '预览：1M 输入 + 1M 输出': 'Preview: 1M input + 1M output',
  '预览': 'Preview',
};
