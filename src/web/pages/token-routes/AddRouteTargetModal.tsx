...
type AddRouteTargetModalProps = {
  ...
  existingTargetAccountIds?: Set<number>;
};

export default function AddRouteTargetModal({
  open,
  onClose,
  routeId,
  routeTitle,
...
}: AddRouteTargetModalProps) {
  const toast = useToast();
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedAccounts, setSelectedAccounts] = useState<Record<number, TargetSelection>>({});
  ...
  const handleSubmit = async () => {
    const targets = Object.values(selectedAccounts);
    if (targets.length === 0) return;

    setSubmitting(true);
    try {
      const result = await api.batchAddTargets(routeId, targets);
      const msg = tr('pages.tokenRoutes.addRouteTargetModal.addedTargets').replace('{count}', String(result.created)) +
        (result.skipped > 0 ? tr('pages.tokenRoutes.addRouteTargetModal.skippedDuplicateTargets').replace('{count}', String(result.skipped)) : '') +
        (result.errors.length > 0 ? tr('pages.tokenRoutes.addRouteTargetModal.targetErrors').replace('{count}', String(result.errors.length)) : '');
      toast.success(msg);
      setSelectedAccounts({});
      setSearchQuery('');
      onSuccess();
      ...
      toast.error(e.message || tr('pages.tokenRoutes.addRouteTargetModal.failedAddTarget'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <CenteredModal
      open={open}
      onClose={handleClose}
      title={`${tr('pages.tokenRoutes.addtargets')} - ${routeTitle}`}
      maxWidth={560}
      footer={
        <div className="flex w-full items-center justify-between gap-3">
          <span className="text-xs text-muted-foreground">
            {tr('pages.downstreamKeys.downstreamKeyEditorModal.selected')} {selectedCount} {tr('pages.tokenRoutes.addRouteTargetModal.targets')}
          </span>
          <div className="flex gap-2">
            ...
            >
              {submitting ? (
                ...
                `${tr('pages.tokenRoutes.addRouteTargetModal.add')} (${selectedCount})`
              )}
            </Button>
          </div>
        </div>
    ...
    >
      <div className="flex flex-col gap-3">
        <SearchInput
          ...
          placeholder={tr('pages.tokenRoutes.addRouteTargetModal.searchaccounts')}
        />
          ...
          {filteredAccounts.length === 0 && filteredMissingAccounts.length === 0 ? (
            <EmptyStateBlock
              title={candidateView.accountOptions.length === 0 && missingAccounts.length === 0
                ? tr('pages.tokenRoutes.addRouteTargetModal.availableAccountsAccountsTokensupportedcallsModel')
                : tr('pages.tokenRoutes.addRouteTargetModal.matchAccounts')}
              className="py-3"
            />
          ) : (
            <>
              {filteredAccounts.map((account) => {
                ...
                const isExisting = existingTargetAccountIds?.has(account.id);
                  ...
                  >
                    <CardContent className="grid gap-2 p-2.5">
                      <div className="flex items-center gap-2">
                        ...
                        {isExisting && (
                          <ToneBadge tone="-muted">{tr('pages.tokenRoutes.addRouteTargetModal.add2')}</ToneBadge>
                        )}
                      </div>

                      {isSelected && tokens.length > 0 && (
                        <div className="ml-6 grid gap-1" onClick={(e) => e.stopPropagation()}>
                          <div className="text-xs text-muted-foreground">{tr('pages.tokenRoutes.addRouteTargetModal.token')}:</div>
                          <ModernSelect
                            size="sm"
                            value={(() => {
                            ...
                            options={[
                              {
                                value: '0',
                                label: tr('pages.tokenRoutes.addRouteTargetModal.accountsdefault'),
                            ...
                            placeholder={tr('pages.tokenRoutes.addRouteTargetModal.selectBindingMode')}
                          />
              ...
              {filteredMissingAccounts.length > 0 && (
                <div className="mt-1 flex flex-col gap-2 border-t border-dashed border-border pt-2">
                  <div className="text-xs text-muted-foreground">
                    {tr('pages.tokenRoutes.addRouteTargetModal.accountsavailableModelToken')}:
                  </div>
                  {filteredMissingAccounts.map((item) => (
                    <Card key={item.accountId} className="border-dashed">
                      ...
                      {onCreateTokenForMissing && (
                        ...
                        >
                          {tr('pages.tokenRoutes.addRouteTargetModal.token2')}
                        </Button>
                      )}
                      </CardContent>
                    </Card>
