    ...
    <div className="grid gap-3 rounded-md border bg-muted/20 p-3" data-testid="failure-backoff-editor">
      ...
      {mode === 'custom' ? (
        <div className="grid gap-3 md:grid-cols-3">
          <label className="grid gap-1.5 text-sm"><span>{tr('pages.tokenRoutes.failureBackoffThreshold')}</span><Input type="number" min={1} max={100} value={policy.failureThreshold} onChange={(event) => updatePolicy({ failureThreshold: Math.max(1, Math.trunc(Number(event.target.value) || 1)) })} /></label>
          <label className="grid gap-1.5 text-sm"><span>{tr('pages.tokenRoutes.failureBackoffLevels')}</span><Input value={policy.levelsSec.join(', ')} onChange={(event) => {
          ...
          <label className="grid gap-1.5 text-sm"><span>{tr('pages.tokenRoutes.failureBackoffMax')}</span><Input type="number" min={1} value={policy.maxSec} onChange={(event) => updatePolicy({ maxSec: Math.max(1, Math.trunc(Number(event.target.value) || 1)) })} /></label>
        </div>
      ) : null}
    </div>
  );
