...
vi.mock('./RouteGraphNodeMenu.js', () => ({
  default: ({ disabled, onSelect }: { disabled?: boolean; onSelect: (type: 'entry') => void }) => (
    <button
      type="button"
      disabled={disabled}
...
describe('RouteGraphWorkspaceView lifecycle', () => {
  ...
  it('creates a primitive from the Index revision and opens its node Focus', async () => {
    state.api.getRouteGraphWorkspaceIndex.mockResolvedValue(indexPage('Macro A'));
    ...
    const createdNode = { id: 'manual:entry:server-issued' };
