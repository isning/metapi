...
describe('compiled runtime artifact contract', () => {
  ...
  it('materializes the same executable plan after compact storage', () => {
    ...
    expect(plan?.affinity).toEqual({
      policy: { kind: 'pool', ttlSec: 1200, crossPoolFallback: 'temporary' },
      pools: [{ id: 'pool:test', label: 'Test pool', executionTargetIds: [91] }],
    });
  });
