...
export function isModelRegexPattern(pattern) {
  return pattern.trim().toLowerCase().startsWith('re:');
}

export function isExactModelPattern(pattern) {
  const normalized = pattern.trim();
  if (!normalized) return false;
  if (isModelRegexPattern(normalized)) return false;
  return !/[\*\?]/.test(normalized);
}

export function parseModelRegexPattern(pattern) {
  if (!isModelRegexPattern(pattern)) {
...
export function matchesModelPattern(model, pattern) {
  ...
  let result;
  if (isModelRegexPattern(normalized)) {
    const parsed = parseModelRegexPattern(normalized);
