...
export type CompiledExecutionSelectionTerm = { termId: string; nodeId?: string | null; mode: 'route' | 'flow' | 'target' | 'execution_attempt' | string; policy: DispatcherPolicy; optionId: string; optionIndex: number; optionKind: 'route' | 'bidirect' | 'target' | 'execution_attempt' | string; enabled: boolean; weight: number; order: number; controlOrder: number; metadata?: Record<string, unknown>; runtime?: Record<string, unknown>; failureBackoff?: import('./routeGraph.js').RouteFailureBackoffOverride; sourceRef?: RouteProgramSourceRef };
