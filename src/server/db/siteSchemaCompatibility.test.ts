  ...
  const inspector: SiteSchemaInspector = {
    ...
    async columnExists(table, column) {
      return existingColumns.has(`${table}.${column}`)
        || (table === 'sites' && existingColumns.has(column));
    },
...
describe('ensureSiteSchemaCompatibility', () => {
  ...
  it('adds the endpoint path mode to an existing endpoint table', async () => {
    const { inspector, executedSql } = createInspector('sqlite', {
      existingTables: ['site_api_endpoints'],
      existingColumns: ['site_api_endpoints.id'],
    });

    await ensureSiteSchemaCompatibility(inspector);

    expect(executedSql).toContain(
      "ALTER TABLE site_api_endpoints ADD COLUMN base_path_mode text NOT NULL DEFAULT 'protocol_default';",
    );
  });
});
