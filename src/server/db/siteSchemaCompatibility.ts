...
export const SITE_COLUMN_COMPATIBILITY_SPECS: SiteColumnCompatibilitySpec[] = [
  {
    ...
    column: 'base_path_mode',
    addSql: {
      sqlite: "ALTER TABLE site_api_endpoints ADD COLUMN base_path_mode text NOT NULL DEFAULT 'protocol_default';",
      mysql: "ALTER TABLE `site_api_endpoints` ADD COLUMN `base_path_mode` VARCHAR(64) NOT NULL DEFAULT 'protocol_default'",
      postgres: "ALTER TABLE \"site_api_endpoints\" ADD COLUMN \"base_path_mode\" TEXT NOT NULL DEFAULT 'protocol_default'",
    },
  },
  {
    column: 'proxy_url',
  ...
  },
  {
    ...
    addSql: {
      sqlite: 'ALTER TABLE sites ADD COLUMN compatibility_policy text;',
      mysql: 'ALTER TABLE `sites` ADD COLUMN `compatibility_policy` TEXT NULL',
      postgres: 'ALTER TABLE "sites" ADD COLUMN "compatibility_policy" TEXT',
    },
  },
  {
...
export async function ensureSiteSchemaCompatibility(inspector: SiteSchemaInspector): Promise<void> {
  ...
  for (const spec of SITE_COLUMN_COMPATIBILITY_SPECS) {
    const table = spec.table || 'sites';
    const hasTable = await inspector.tableExists(table);
    if (!hasTable) continue;
    const hasColumn = await inspector.columnExists(table, spec.column);
    if (!hasColumn) {
