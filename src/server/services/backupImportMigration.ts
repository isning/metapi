...
function legacyModelRewrite(route: JsonRecord, modelName: string): string | undefined {
  const raw = route.modelMapping ?? route.model_mapping;
  if (!raw) return undefined;
  let parsed: unknown = raw;
  if (typeof raw === 'string') {
    try { parsed = JSON.parse(raw); } catch { throw new Error(`Cannot migrate model mapping for ${modelName}: invalid JSON`); }
  }
  if (!isRecord(parsed)) throw new Error(`Cannot migrate model mapping for ${modelName}: expected an object`);
  const entries = Object.entries(parsed).flatMap(([pattern, value]) => (
    typeof value === 'string' && value.trim() ? [[pattern, value.trim()] as const] : []
  ));
  const exact = entries.find(([pattern]) => pattern === modelName);
  if (exact && isExactModelPattern(modelName)) return exact[1].trim();
  if (entries.length === 0) return undefined;
  throw new Error(`Cannot migrate model mapping for ${modelName}: non-exact mappings require a Graph rewrite rule`);
}
...
function convertPreviousTokenRoutes(input: {
  ...
  const ensureTarget = (endpoint: JsonRecord, modelName: string):
    ...
    const oauthRouteUnitId = readPositiveInteger(endpoint, 'oauthRouteUnitId', 'oauth_route_unit_id');
    const key = executionTargetKey(modelName, accountId, tokenId, oauthRouteUnitId);
  ...
  const groups = routes.flatMap((route, index): LegacyFacadeGroup[] => {
    ...
    const group: LegacyFacadeGroup = {
      ...
      modelRewrite: legacyModelRewrite(route, modelName || displayName),
      stages: [],
    };
  ...
  for (const group of groups) {
    ...
    const created = createRouteGroupFacadeMacro(source, {
      ...
      ...((group.filters || group.modelRewrite) ? {
        filters: {
          operations: [
            ...(group.filters?.operations || []),
            ...(group.modelRewrite ? [{ type: 'rewrite_model' as const, source: 'current_model' as const, operation: 'set' as const, value: group.modelRewrite }] : []),
          ],
        },
      } : {}),
