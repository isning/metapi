...
describe('backup import dispatch policy migration', () => {
  ...
  it('converts an exact legacy model mapping into a route-scoped model rewrite', () => {
    const migrated = migratePreviousRouteBackupToCurrentRuntime(legacySection(), {
      tokenRoutes: [{ id: 7, modelPattern: 'public-model', modelMapping: { 'public-model': 'upstream-model' } }],
      routeEndpointTargets: [{ routeId: 7, accountId: 1, modelName: 'upstream-model' }],
    });

    expect(migrated.graphSource?.macros?.[0]?.config.filters?.operations).toContainEqual(expect.objectContaining({
      type: 'rewrite_model', source: 'current_model', operation: 'set', value: 'upstream-model',
    }));
  });
