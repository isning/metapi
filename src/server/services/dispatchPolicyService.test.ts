...
describe('dispatchPolicyService', () => {
  ...
  it('rejects CEL syntax errors before a policy can enter the registry', () => {
    const validation = validateDispatchPolicyRegistry({
      defaultPolicyId: 'invalid-syntax',
      policies: [{
        id: 'invalid-syntax',
        name: 'Invalid syntax',
        kind: 'cel',
        selectionMode: 'direct',
        selectExpression: 'request.payload.tier ==',
      }],
    });

    expect(validation.value).toBeNull();
    expect(validation.errors.join(' ')).toMatch(/could not be compiled/i);
  });

  it('rejects CEL eligibility expressions that return a non-boolean value', () => {
    const validation = validateDispatchPolicyRegistry({
      defaultPolicyId: 'invalid-eligibility',
      policies: [{
        id: 'invalid-eligibility',
        name: 'Invalid eligibility',
        kind: 'cel',
        selectionMode: 'weighted',
        eligibilityExpression: 'self.weight',
        contributionExpression: 'self.weight',
      }],
    });

    expect(validation.value).toBeNull();
    expect(validation.errors.join(' ')).toMatch(/boolean/i);
  });
