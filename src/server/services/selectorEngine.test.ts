...
describe('selectorEngine', () => {
  ...
  it('fails closed with a diagnostic error when CEL evaluation is invalid', () => {
    ...
    expect(() => selectRuntimeCandidate({
      selectorId: 'route.invalid-eligibility-result',
      policy: { strategy: 'policy_weighted', eligibility: 'self.weight', contribution: 'self.weight' },
      candidates,
    })).toThrow(/route.invalid-eligibility-result.*eligibility/i);
    expect(() => selectRuntimeCandidate({
      selectorId: 'route.invalid-order-result',
      policy: { strategy: 'policy_ordered', order: 'self.metadata.missing' },
      candidates,
    })).toThrow(/route.invalid-order-result.*order/i);
  });
