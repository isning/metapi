...
export async function consumeManagedKeyRequest(keyId: number): Promise<boolean> {
  ...
  const result = await db.update(schema.downstreamApiKeys).set({
    ...
    eq(schema.downstreamApiKeys.enabled, true),
    or(isNull(schema.downstreamApiKeys.expiresAt), gt(schema.downstreamApiKeys.expiresAt, nowIso)),
    or(
      isNull(schema.downstreamApiKeys.maxCost),
      sql`coalesce(${schema.downstreamApiKeys.usedCost}, 0) < ${schema.downstreamApiKeys.maxCost}`,
    ),
    or(
      isNull(schema.downstreamApiKeys.maxRequests),
      sql`coalesce(${schema.downstreamApiKeys.usedRequests}, 0) < ${schema.downstreamApiKeys.maxRequests}`,
    ),
  )).run();
