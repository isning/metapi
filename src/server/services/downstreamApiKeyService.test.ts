...
describe('downstreamApiKeyService', () => {
  ...
  it('accumulates managed key request/cost usage and applies limits', async () => {
    ...
    expect(await service.consumeManagedKeyRequest(row.id)).toBe(true);
    expect(await service.consumeManagedKeyRequest(row.id)).toBe(true);
    expect(await service.consumeManagedKeyRequest(row.id)).toBe(false);
  ...
  it('atomically enforces the managed-key request limit under concurrency', async () => {
    const row = await db.insert(schema.downstreamApiKeys).values({
      name: 'concurrent-metered-key',
      key: 'sk-concurrent-metered-key',
      enabled: true,
      maxRequests: 3,
      usedRequests: 0,
    }).returning().get();
    ...
    const results = await Promise.all(
      Array.from({ length: 12 }, () => service.consumeManagedKeyRequest(row.id)),
    );

    expect(results.filter(Boolean)).toHaveLength(3);
    const latest = await service.getDownstreamApiKeyById(row.id);
    expect(latest?.usedRequests).toBe(3);
  });
