...
import { CURRENT_CONFIG_VERSION, migratePublishedMainPreferenceSettings } from './configMigrationService.js';
import { replaceRouteGraphSourceTransaction, RouteGraphSyncValidationError } from './routeGraphService.js';
import { recordSiteCatalogMutation } from './siteCatalogMutationService.js';
import { migrateImportedRouteGraphSourceJson, migratePreviousRouteBackupToCurrentRuntime, type BackupImportNotice, type BackupImportRouteRuntimeMigrationResult } from './backupImportMigration.js';
import { migrateImportedAccountCredential, reconcileImportedAccountCredentialTokens, type ImportedAccountCredential } from './backupAccountCredentialMigration.js';
...
const EXCLUDED_SETTING_KEYS = new Set<string>(['auth_token', 'db_type', 'db_url', 'db_ssl']);
...
function normalizeBackupWebdavConfig(raw: unknown): BackupWebdavConfig {
  ...
  const autoSyncCron = typeof source.autoSyncCron === 'string' && cron.validate(source.autoSyncCron) ? source.autoSyncCron : BACKUP_WEBDAV_DEFAULT_AUTO_SYNC_CRON;
  return {
...
async function* jsonArrayFromIdPages(table: any): AsyncGenerator<string> {
  ...
  yield '[';
  while (true) {
    const rows = await db.select().from(table).where(gt(table.id, afterId)).orderBy(asc(table.id)).limit(BACKUP_EXPORT_PAGE_SIZE).all();
...
async function* preferencesJson(): AsyncGenerator<string> {
  ...
  yield '{"settings":[';
  while (true) {
    const rows = await db.select().from(schema.settings).where(gt(schema.settings.key, afterKey)).orderBy(asc(schema.settings.key)).limit(BACKUP_EXPORT_PAGE_SIZE).all();
    if (rows.length === 0) break;
    for (const row of rows) {
      afterKey = row.key;
  ...
  if (!sawPlatformPricingConfig) {
    ...
    if (row) {
      const value =
        legacyRoutingFallbackUnitCost === null
          ? row.value
          : {
...
async function* accountsJson(): AsyncGenerator<string> {
  ...
  const activeVersion = await db.select().from(schema.routeGraphActiveVersion).where(eq(schema.routeGraphActiveVersion.id, 1)).get();
...
const NATIVE_BACKUP_DEFERRED_ARRAY_PATHS = new Set<NativeBackupArrayPath>(['accounts.credentialEndpointBindings', 'accounts.runtimeExecutionTargets', 'accounts.runtimeExecutionTargetState']);
...
function updateJsonPathStackBeforeToken(stack: Array<string | number | null>, previousToken: string, chunk: { name: string; value?: unknown }): void {
  switch (chunk.name) {
    case 'startObject':
    case 'startArray':
    case 'startString':
...
function updateJsonPathStackAfterToken(stack: Array<string | number | null>, chunk: { name: string }): void {
...
async function streamNativeBackupJson(
  ...
  let capture: {
    path: string;
    assembler: InstanceType<typeof Assembler>;
  } | null = null;

  createReadStream(filePath).pipe(parser);
  for await (const chunk of parser as AsyncIterable<{
    name: string;
    value?: unknown;
  }>) {
    ...
    if (capture) {
      ...
      const arrayPath = typeof stack.at(-1) === 'number' ? pathFromStack(stack.slice(0, -1)) : null;
      const capturePath = arrayPath && arrayPaths.has(arrayPath) ? arrayPath : directValuePaths.has(path) ? path : null;
      ...
      } else if (capturePath && ['nullValue', 'trueValue', 'falseValue', 'stringValue', 'numberValue'].includes(chunk.name)) {
...
async function importNativeBackupFromJsonFile(filePath: string): Promise<BackupImportResult | null> {
  ...
  try {
    await replaceRouteGraphSourceTransaction({
      ...
      replace: async (tx) => {
        ...
        const flush = async (path: NativeBackupArrayPath, table: any, mapRow: (row: unknown) => unknown = (row) => row) => {
          ...
          await tx
            .insert(table)
            .values(batch.map(mapRow) as any)
            .run();
        };
        ...
        const tables = new Map<NativeBackupArrayPath, [any, (row: unknown) => unknown]>([
          ...
          [
            'accounts.routeGraph.versions',
            [
              schema.routeGraphVersions,
              (row) =>
                isRecord(row)
                  ? {
                      ...row,
                      sourceGraphJson: migrateImportedRouteGraphSourceJson(String(row.sourceGraphJson || '')),
                    }
                  : row,
            ],
          ],
          [
            'accounts.routeGraph.drafts',
            [
              schema.routeGraphDrafts,
              (row) =>
                isRecord(row)
                  ? {
                      ...row,
                      workingGraphJson: migrateImportedRouteGraphSourceJson(String(row.workingGraphJson || '')),
                    }
                  : row,
            ],
          ],
        ...
        await streamNativeBackupJson(filePath, {
          onArrayStart: async (path) => {
            ...
            if (seenArrays.has('accounts.accountTokens') && path !== 'accounts.accountTokens') {
          ...
          onValue: async (path, value) => {
            ...
            const maxRows = Math.max(1, Math.min(BACKUP_IMPORT_MAX_ROWS, Math.floor(BACKUP_IMPORT_MAX_PARAMETERS / columnCount)));
            if (table && !NATIVE_BACKUP_DEFERRED_ARRAY_PATHS.has(arrayPath) && batch.length >= maxRows) await flush(arrayPath, table[0], table[1]);
          },
        });
        ...
        if (accountsRequested && !['accounts.sites', 'accounts.accounts', 'accounts.accountTokens'].every((path) => seenArrays.has(path as NativeBackupArrayPath))) {
        ...
        if (preferencesRequested) {
          const migrated = migratePublishedMainPreferenceSettings([]).settings.filter(
            (row) =>
          ...
          for (const row of migrated) {
            const value =
              row.key === 'platform_pricing_config_v1' && legacyRoutingFallbackUnitCost !== null
                ? {
                    ...(row.value as Record<string, unknown>),
                    upstreamDefaultPricing: {
                      inputPerMillion: legacyRoutingFallbackUnitCost,
        ...
        if (accountsRequested) await recordSiteCatalogMutation(tx);
        return {
          sourceGraph: accountsRequested ? graphSource : null,
          result: undefined,
        };
      },
    });
    ...
    if (error instanceof RouteGraphSyncValidationError) {
      throw new Error(`导入的历史路由无法编译：${error.diagnostics.map((item) => item.message).join('；')}`);
    }
    throw error;
  }
...
export async function importBackupFromGzipStream(input: Readable, maxUncompressedBytes: number): Promise<BackupImportResult> {
...
export async function importBackupFromJsonStream(input: Readable, maxUncompressedBytes: number): Promise<BackupImportResult> {
  const directory = await mkdtemp(join(tmpdir(), 'metapi-backup-import-'));
  const filePath = join(directory, 'backup.json');
  try {
...
async function restoreRouteGraph(tx: any, routeGraph: BackupRouteGraphSection | undefined): Promise<void> {
  ...
  await insertRows(
    tx,
    schema.routeGraphVersions,
    (routeGraph.versions || []).map((row: any) => ({
      ...row,
  ...
  if (routeGraph.activeVersion) {
    await tx
      .insert(schema.routeGraphActiveVersion)
      .values(routeGraph.activeVersion as any)
      .run();
  }
}

function activeSourceFromBackupRouteGraph(routeGraph: BackupRouteGraphSection | undefined): RouteGraphSource | null {
  ...
  try {
    await replaceRouteGraphSourceTransaction({
      ...
      replace: async (tx) => {
        ...
        await recordSiteCatalogMutation(tx);
        return { sourceGraph: graphSource, result: undefined };
      },
    });
  } catch (error) {
    if (error instanceof RouteGraphSyncValidationError) {
...
export async function importBackup(data: RawBackupData): Promise<BackupImportResult> {
  ...
  return {
    ...
    appliedSettings,
    summary:
      accountsImported && accountsSection
        ? {
...
export async function getBackupWebdavConfig() {
  const [config, state] = await Promise.all([loadBackupWebdavConfig(), loadBackupWebdavState()]);
  return {
    success: true,
...
export async function saveBackupWebdavConfig(
  input: Partial<BackupWebdavConfig> & {
    password?: string;
  ...
  const next: BackupWebdavConfig = {
    ...
    password: input.clearPassword ? '' : input.password !== undefined ? String(input.password) : existing.password,
    ...
    autoSyncCron: typeof input.autoSyncCron === 'string' && input.autoSyncCron.trim() ? input.autoSyncCron.trim() : existing.autoSyncCron,
  };
