...
export const ROUTE_FAILURE_COOLDOWN_MAX_SEC_CEILING = 30 * 24 * 60 * 60;
export const ROUTE_RUNTIME_CACHE_TTL_MS_DEFAULT = 1_500;
...
function preferredEnv(env: NodeJS.ProcessEnv, current: string, legacy: string): string | undefined {
...
export function normalizeRouteFailureCooldownMaxSec(value: unknown): number | null {
...
export function normalizeRouteRuntimeCacheTtlMs(value: unknown): number {
  const normalized = Number(value);
  if (!Number.isFinite(normalized) || normalized <= 0) return ROUTE_RUNTIME_CACHE_TTL_MS_DEFAULT;
  return Math.max(100, Math.min(60_000, Math.trunc(normalized)));
}
...
export function buildConfig(env: NodeJS.ProcessEnv) {
  ...
  return {
    ...
    proxySessionTargetConcurrencyLimit: Math.max(0, Math.trunc(parseNumber(preferredEnv(env, 'PROXY_SESSION_TARGET_CONCURRENCY_LIMIT', 'PROXY_SESSION_CHANNEL_CONCURRENCY_LIMIT'), 2))),
    proxySessionTargetQueueWaitMs: Math.max(0, Math.trunc(parseNumber(preferredEnv(env, 'PROXY_SESSION_TARGET_QUEUE_WAIT_MS', 'PROXY_SESSION_CHANNEL_QUEUE_WAIT_MS'), 1_500))),
    ...
    proxyDebugFilterSessionId: (preferredEnv(env, 'PROXY_DEBUG_FILTER_SESSION_ID', 'PROXY_DEBUG_TARGET_SESSION_ID') || '').trim(),
    proxyDebugFilterClientKind: (preferredEnv(env, 'PROXY_DEBUG_FILTER_CLIENT_KIND', 'PROXY_DEBUG_TARGET_CLIENT_KIND') || '').trim(),
    proxyDebugFilterModel: (preferredEnv(env, 'PROXY_DEBUG_FILTER_MODEL', 'PROXY_DEBUG_TARGET_MODEL') || '').trim(),
