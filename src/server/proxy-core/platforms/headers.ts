...
export function buildCodexRuntimeHeaders(input: {
  ...
  const originator = inferCodexOfficialOriginator(input.platformHeaders)
    || inferCodexOfficialOriginator(input.baseHeaders)
    || getInputHeader(input.platformHeaders, 'originator')
    || input.originatorDefault
    || 'codex_cli_rs';
  const accountId = getInputHeader(input.platformHeaders, 'chatgpt-account-id');
...
export function buildGeminiCliRuntimeHeaders(input: {
  ...
  const apiClient = (
    getInputHeader(input.platformHeaders, 'x-goog-api-client')
    || getInputHeader(input.baseHeaders, 'x-goog-api-client')
  );
  const userAgent = buildGeminiCliUserAgent(
    input.modelName,
    getInputHeader(input.platformHeaders, 'user-agent') || getInputHeader(input.baseHeaders, 'user-agent'),
  );
...
export function getCodexSessionHeaderValue(headers: Record<string, string | string[] | undefined | unknown>): string {
