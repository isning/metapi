...
import {
  resolveAntigravityPlatformAction,
...
describe('resolveAntigravityPlatformAction', () => {
  it('routes Claude-family non-stream requests through streamGenerateContent', () => {
    expect(resolveAntigravityPlatformAction('generateContent', false, 'claude-opus-4-1'))
      .toBe('streamGenerateContent');
  });

  it('routes Gemini special-model non-stream requests through streamGenerateContent', () => {
    expect(resolveAntigravityPlatformAction('generateContent', false, 'gemini-3-pro-latest'))
      .toBe('streamGenerateContent');
    expect(resolveAntigravityPlatformAction('generateContent', false, 'gemini-3.1-flash-image'))
      .toBe('streamGenerateContent');
  });
});
