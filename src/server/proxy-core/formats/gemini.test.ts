...
describe('gemini transformer-owned path parsing', () => {
  it('keeps apiVersion and modelActionPath parsing in transformer helpers', async () => {
    const geminiAdapter = readWorkspaceFile('src/server/proxy-core/formats/gemini.ts');

    expect(geminiAdapter).not.toContain('function resolveGeminiApiVersion(');
    expect(geminiAdapter).not.toContain('function extractGeminiModelActionPath(');
