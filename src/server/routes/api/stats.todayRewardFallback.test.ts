...
describe('stats dashboard today reward fallback', () => {
  ...
  it('repairs ISO timestamps for dashboard today reward filtering', async () => {
    const site = await db.insert(schema.sites).values({
      name: 'stats-iso-site',
      url: 'https://stats-iso.example.com',
      platform: 'new-api',
    }).returning().get();
    const account = await db.insert(schema.accounts).values({
      siteId: site.id,
    ...
    expect(response.statusCode).toBe(200);
    const body = response.json() as { todayReward: number };
    expect(body.todayReward).toBe(3.6);
  });
  ...
  it('counts dashboard today spend only inside local-day range', async () => {
    ...
    await Promise.all([
      insertCanonicalTerminalRequest({ id: 'dashboard-spend-before', siteId: site.id, accountId: account.id, completedAt: beforeStart, amount: 1 }),
      insertCanonicalTerminalRequest({ id: 'dashboard-spend-in-range', siteId: site.id, accountId: account.id, completedAt: inRange, amount: 3 }),
      insertCanonicalTerminalRequest({ id: 'dashboard-spend-after', siteId: site.id, accountId: account.id, completedAt: afterEnd, amount: 5 }),
    ]);
  ...
  it('repairs ISO timestamps for dashboard today spend filtering', async () => {
    ...
    await insertCanonicalTerminalRequest({ id: 'dashboard-spend-iso', siteId: site.id, accountId: account.id, completedAt: inRangeIso, amount: 6.4 });
    await repairStoredCreatedAtValues();
    await runUsageAggregationProjectionPass();
